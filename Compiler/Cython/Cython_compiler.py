...
class Compiler:
    ...
    def list_all_directories(self, root):
        directory_list = []
    ...
    def compile_code(self, file_name):
        if self.do_compile == True:
            if self.use_make == False:
                ...
                cmd +=  [
                            '-T', f'./{file_name}/{file_name}.ld'
                        ]
                
                include_dirs = [
                            r'./include',
                            r'C:\Users\QC109_4\anaconda3\include'
                        ]
                for include_dir in include_dirs:
                    dir_list = self.list_all_directories(include_dir)
                    for dir_comp in dir_list:
                        cmd.extend(["-I", dir_comp])
                    
                if self.is_cpp == True:
                    cmd += [f'./{file_name}/{file_name}.cpp']
                else:
